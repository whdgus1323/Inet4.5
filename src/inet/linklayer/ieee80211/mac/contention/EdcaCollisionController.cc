...
void EdcaCollisionController::expectedChannelAccess(Edcaf *edcaf, simtime_t time)
{
    ...
    std::cout << "The expected channel access of the " << printAccessCategory(ac) << " queue is: " << time << std::endl;
    ...
    if (txStartTimes[accessCategory] == now) {
        for (int ac = accessCategory + 1; ac < 4; ac++) {
            if (txStartTimes[ac] == now) {
                std::cerr << "Internal collision detected between multiple access categories for the current simulation time.\n";
                return true;
            }
        }
    }
