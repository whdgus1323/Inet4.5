...
void Contention::startContention(int cw, simtime_t ifs, simtime_t eifs, simtime_t slotTime, ICallback *callback)
{
    ...
    appendContentionMetric("time=" + simTime().str() +
            ", node=" + std::string(getContainingNode(this)->getFullName()) +
            ", event=BACKOFF_START, cw=" + std::to_string(cw) +
            ", slots=" + std::to_string(backoffSlots) +
            ", ifs=" + ifs.str() +
            ", eifs=" + eifs.str() +
            ", slotTime=" + slotTime.str());
...
void Contention::mediumStateChanged(bool mediumFree)
{
    ...
    appendContentionMetric("time=" + simTime().str() +
            ", node=" + std::string(getContainingNode(this)->getFullName()) +
            ", event=MEDIUM_" + std::string(mediumFree ? "FREE" : "BUSY") +
            ", state=" + std::string(fsm.getStateName()));
...
void Contention::handleMessage(cMessage *msg)
{
    if (msg == startTxEvent) {
        ...
        appendContentionMetric("time=" + simTime().str() +
                ", node=" + std::string(getContainingNode(this)->getFullName()) +
                ", event=CHANNEL_GRANTED, contentionTime=" + (simTime() - startTime).str());
...
void Contention::scheduleTransmissionRequest()
{
    ...
    appendContentionMetric("time=" + simTime().str() +
            ", node=" + std::string(getContainingNode(this)->getFullName()) +
            ", event=SCHEDULE_TX, waitInterval=" + waitInterval.str() +
            ", useEifs=" + std::string(useEifs ? "true" : "false") +
            ", backoffSlots=" + std::to_string(backoffSlots));
...
std::string Contention::findPwdParameter() const
{
    for (cModule *module = getParentModule(); module != nullptr; module = module->getParentModule()) {
        if (module->hasPar("pwd"))
            return module->par("pwd").stdstringValue();
    }
    return "";
}

void Contention::appendContentionMetric(const std::string& line) const
{
    if (pwd.empty())
        return;
    std::ofstream logFile(pwd + "/contention_log.txt", std::ios::app);
    logFile << line << endl;
}
