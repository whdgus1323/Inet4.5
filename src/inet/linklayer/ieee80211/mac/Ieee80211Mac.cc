...
struct NgvRadioEnvRequestVectorGlobal {
    int ppduFormat = 1;
    int mcs = 3;
    int nss = 1;
...
struct NgvRadioEnvStatusVectorGlobal {
    int ppduFormat = 1;
    int mcs = 3;
    bool aggregation = true;
    int channel = 180;
    int primaryChannel = 180;
    int channelWidth = 20;
    double rssi = -85;
};

struct MlmeRadioEnvironmentState {
    int stationCount = 0;
    int ngvCapabilityPercent = 100;
    int channelBusyPercent = 0;
};

static std::unordered_map<cModule*, NgvRadioEnvRequestVectorGlobal> g_requestVec;
static std::unordered_map<cModule*, NgvRadioEnvStatusVectorGlobal>  g_statusVec;
static std::unordered_map<cModule*, MlmeRadioEnvironmentState>      g_mlmeState;
...
Ieee80211Mac::~Ieee80211Mac()
{
    ...
    g_requestVec.erase(host);
    g_statusVec.erase(host);
    g_mlmeState.erase(host);
}
...
void Ieee80211Mac::handleUpperPacket(Packet *packet)
{
            ...
            mib->bssStationData.stationType == Ieee80211Mib::ACCESS_POINT)
    {
        ...
        if (!receiverAddress.isMulticast()) {
            ...
            {
                EV << "STA with MAC address " << receiverAddress
...
void Ieee80211Mac::encapsulate(Packet *packet)
{
    ...
    else if (mib->mode == Ieee80211Mib::OCB) {
        header->setReceiverAddress(destAddress);
        header->setTransmitterAddress(mib->address);
        header->setAddress3(MacAddress::BROADCAST_ADDRESS); // wildcard BSSID
    }
...
void Ieee80211Mac::receiveSignal(cComponent *source, simsignal_t signalID, intval_t value, cObject *details)
{
    ...
    // 수신된 신호의 일부가 변경되었음을 알리는 신호. 이는 신호의 특정 부분이 변경되었거나 새로운 신호가 도착한 경우 발생.
...
void Ieee80211Mac::processUpperFrame(Packet *packet, const Ptr<const Ieee80211DataOrMgmtHeader>& header)
{
    ...
    bool applyRepetition = !strcmp(packet->getName(), "aodv::Rreq");

    int numTx = applyRepetition ? bdRepetitions : 1;
    if (numTx < 1)
        numTx = 1;
    ...
    for (int i = 0; i < numTx - 1; i++) {
        ...
        if (mib->qos)
            hcf->processUpperFrame(copy, copy->peekAtFront<Ieee80211DataOrMgmtHeader>());
        else
            dcf->processUpperFrame(copy, copy->peekAtFront<Ieee80211DataOrMgmtHeader>());
    }
...
void Ieee80211Mac::processUpperFrame(Packet *packet, const Ptr<const Ieee80211DataOrMgmtHeader>& header)
{
    ...
    // RREQ repetition is handled by the DCF frame sequence so channel contention happens only once.
    if (mib->qos)
        hcf->processUpperFrame(packet, header);
    else
        dcf->processUpperFrame(packet, header);
