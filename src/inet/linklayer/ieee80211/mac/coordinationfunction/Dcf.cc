...
Define_Module(Dcf); // @suppress("Abstract class cannot be instantiated")
...
void Dcf::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LINK_LAYER) {
        ...
        //if (hasPar("pwd"))   pwd   = par("pwd").stdstritalue();
...
void Dcf::processUpperFrame(Packet *packet, const Ptr<const Ieee80211DataOrMgmtHeader>& header) // @suppress("Member declaration not found")
{
...
void Dcf::processMgmtFrame(Packet *packet, const Ptr<const Ieee80211MgmtHeader>& mgmtHeader) // @suppress("Member declaration not found")
{
...
void Dcf::originatorProcessTransmittedFrame(Packet *packet)
{
    ...
    if (auto dataOrMgmtHeader = dynamicPtrCast<const Ieee80211DataOrMgmtHeader>(transmittedHeader)) {
        ...
        auto protectedFrame = channelAccess->getInProgressFrames()->getFrameToTransmit();
...
void Dcf::originatorProcessFailedFrame(Packet *failedPacket)
{
    ...
    AckHandler::Status ackStatus = ackHandler->getAckStatus(failedHeader);
    int ackStatusValue = static_cast<int>(ackStatus);

    std::string packetName = failedPacket->getName();
