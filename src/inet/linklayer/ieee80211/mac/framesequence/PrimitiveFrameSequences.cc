...
void BroadcastRreqFs::startSequence(FrameSequenceContext *context, int firstStep)
{
    this->firstStep = firstStep;
    step = 0;
}
...
{
    if (step >= repetitionCount)
        return nullptr;

    auto packet = context->getInProgressFrames()->getFrameToTransmit();
    bool isLastTransmission = step == repetitionCount - 1;
    // The initial broadcast follows the normal sequence IFS, but 802.11bd repetitions
    // after the first transmission must be sent with SIFS spacing.
    auto interFrameSpace = step == 0 ? context->getIfs() : context->getSifs();

    // Let DCF own cleanup for all transmitted frames because it already drops and deletes them.
    // Early repetitions use duplicates, while the final transmission uses the original in-progress frame.
    return new TransmitStep(isLastTransmission ? packet : packet->dup(), interFrameSpace, false);
}

bool BroadcastRreqFs::completeStep(FrameSequenceContext *context)
{
    if (step < repetitionCount) {
        step++;
        return true;
