...
void PingApp::handleMessageWhenUp(cMessage *msg)
{
    ...
    if (strcmp(msg->getName(), "AODV message") == 0) {



        /*
        if (!isStartTimeUpdated) {
            std::cout << "Received AODV message, updating startTime" << std::endl;
            lastRerrTime = simTime().dbl();

            stopTime += (simTime() - SimTime(30, SIMTIME_S));

            if (rreqCountingEnabled) {
                rreqSeqNo = sendSeqNo;
                sentCount = 0;
                sendSeqNo = 0;
                numPongs = 0;
    ...
    if (strcmp(msg->getName(), "RERR message") == 0) {

        /*
        isRerrState = true;

        SimTime rerrDuration = simTime() - lastRerrTime;
        stopTime += rerrDuration;
        lastRerrTime = simTime().dbl();
        //std::cout << simTime() << " : " << stopTime << std::endl;

        std::ofstream logFile;
        logFile.open("data/rerrPing.txt", std::ios_base::app);
        logFile << simTime().dbl() << endl;
        logFile.close();
         */
...
void PingApp::socketDataArrived(INetworkSocket *socket, Packet *packet)
{
    ...
    if (packet->getTag<PacketProtocolTag>()->getProtocol() == &Protocol::icmpv4) {
        ...
        if (icmpHeader->getType() == ICMP_ECHO_REPLY) {
            ...
            simtime_t CT = simTime();

            /*
            std::ofstream logFileReply(pwd + "/Reply.txt", std::ios::app);
            logFileReply << CT << ":" << echoReply->getSeqNumber() << std::endl;
            logFileReply.close();
             */
...
void PingApp::sendPingRequest()
{
     ...
     */
    /*
    std::ofstream logFileSend(pwd + "/Send.txt", std::ios::app);
    logFileSend << CT << ":" << sendSeqNo << std::endl;
    logFileSend.close();
     */
    ...
    // --------------- 802.11bd 구현 추가 ---------------
    {
        auto ngvReq = outPacket->addTag<inet::NgvRadioEnvironmentReqTag>();

        ngvReq->setPpduFormatPresent(true);
        ngvReq->setPpduFormat(1);

        ngvReq->setMcsPresent(true);
        ngvReq->setNgvMcs(2);

        ngvReq->setRepetitionPresent(true);
        ngvReq->setNumRepetitions(3);

        ngvReq->setExpiryPresent(true);
        ngvReq->setExpiryTime(10.0);
    }
    // --------------- 802.11bd 구현 추가 ---------------
...
void PingApp::finish()
{
    ...
    simtime_t duration = simTime() - lastStart;
    if (duration > SIMTIME_ZERO) {
        double throughput_bps = (double)(packetSize * numPongs * 8) / duration.dbl();
        recordScalar("Throughput (bps)", throughput_bps);
    }
    ...
    std::string pingPath = pwd + std::string("/result.txt");
    std::ifstream chk(pingPath);
    bool exits = chk.good();
    chk.close();

    std::ofstream out(pingPath, std::ios::app);
    out << "--------------------------------------------------------" << endl;
    out << "\t" << getFullPath() << endl;
    out << "--------------------------------------------------------" << endl;
    out << "sent: " << sendSeqNo << "   received: " << numPongs << "   loss rate (%): " << 100 * (lossCount / (double)sendSeqNo) << endl;
    out << "sent: " << sentCount << "   received: " << numPongs << "   PDR (%): " << 100 * ((double)numPongs / (double)sentCount) << endl;
    out << "round-trip min/avg/max (ms): " << (rttStat.getMin() * 1000.0) << "/"
            << (rttStat.getMean() * 1000.0) << "/" << (rttStat.getMax() * 1000.0) << "/" << (rttStat.getStddev() * 1000.0) << endl;
    out << "stddev (ms): " << (rttStat.getStddev() * 1000.0) << "   variance:" << rttStat.getVariance() << endl;

    // === Throughput 출력 추가 ===
    if (duration > SIMTIME_ZERO) {
        double throughput_bps = (double)(packetSize * numPongs * 8) / duration.dbl();
        out << "Throughput (bps): " << throughput_bps << endl;
    }
    out << "--------------------------------------------------------" << endl;
    out.close();
