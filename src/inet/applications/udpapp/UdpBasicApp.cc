...
void UdpBasicApp::finish()
{
    ...
    std::cout << "packets sent : " << numSent << std::endl;
    std::cout << "packets received : " << numReceived << std::endl;

    double pdr = (numSent > 0) ? static_cast<double>(numReceived) / numSent : 0.0;
...
void UdpBasicApp::sendPacket()
{
    ...
    //std::cout << "[DEBUG] destAddr = " << destAddr.str() << std::endl;
    std::cout << "[SEND] " << packet->getName() << " to " << destAddresses[0] << std::endl;
...
void UdpBasicApp::processStart()
{
    ...
    std::cout << "[DEBUG] BIND DONE at port = " << localPort << " (node = " << getParentModule()->getFullPath() << ")" << std::endl;
...
void UdpBasicApp::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    ...
    //std::cout << "[DEBUG] socketDataArrived called\n";
