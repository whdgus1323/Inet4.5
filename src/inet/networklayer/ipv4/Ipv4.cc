...
void Ipv4::datagramLocalOut(Packet *packet)
{
    ...
    else { // unicast and broadcast : 전송실패 ping
           // check for local delivery

/*        if(strstr(packet->getName(), "ping") != nullptr) {
             std::ofstream outFile("data/11.txt", std::ios::app);
           outFile << packet->getName() << std::endl;
           outFile.close();
        }*/
    ...
    if (!destIE) { // no route found
        ...
        if (fromIE != nullptr) {
            ...
            if (currentTime - lastPrintTime >= 1.0) {
                if(strstr(packet->getName(), "ping") != nullptr) {
                    std::ofstream outFile("data/no_fromIE.txt", std::ios::app);
                    outFile << currentTime << " : " << packet->getName() << std::endl;
                    outFile.close();
                }
            }*/
        }
