...
/*
void printPacketHeaders(const inet::Packet *packet, std::ostream& out) {
    out << "Printing all headers in the packet:" << std::endl;

    auto data = packet->peekData();
    inet::b offset = inet::b(0);
    auto totalLength = packet->getTotalLength();

    while (offset < totalLength) {
        auto chunk = data->peek(offset, totalLength - offset);
        out << "Chunk type: " << chunk->getClassName() << ", content: " << chunk->str() << " : ";

        if (auto sequenceChunk = dynamicPtrCast<const inet::SequenceChunk>(chunk)) {
            for (const auto& subChunk : sequenceChunk->getChunks()) {
                out << "Sub-chunk type: " << subChunk->getClassName() << ", content: " << subChunk->str() << " : ";
            }
        }
...
const IReceptionResult *RadioMedium::computeReceptionResult(const IRadio *radio, const IListening *listening, const ITransmission *transmission) const
{
    ...
    auto transmitterPacket = const_cast<inet::Packet *>(transmission->getPacket());
    auto chunk = transmitterPacket->peekData<inet::Chunk>();
    simtime_t latency = transmission->getEndTime() - transmission->getStartTime();

    std::ofstream outFile("packet_headers.txt", std::ios::app);
    if (receptionDecision->isReceptionSuccessful() == 1) {
        outFile << transmitterPacket << " : ";
...
const IReceptionResult *RadioMedium::computeReceptionResult(const IRadio *radio, const IListening *listening, const ITransmission *transmission) const
{
    ...
    // Reception decision check and logging



    if (!receptionDecision->isReceptionSuccessful()) {
        std::ofstream outFile("failed_packets.txt", std::ios::app);
        outFile << "Failed packet: " << transmitterPacket << " : ";
        printPacketHeaders(transmitterPacket, outFile);
        outFile << "Latency: " << latency << " : "
                << "ScalarSnir, minSNIR = " << snir->getMean() << " : "
                << reception << std::endl;
    } else {
        std::ofstream outFile("successful_packets.txt", std::ios::app);
        outFile << "Successful packet: " << transmitterPacket << " : ";
        printPacketHeaders(transmitterPacket, outFile);
        outFile << "Latency: " << latency << " : "
                << "ScalarSnir, minSNIR = " << snir->getMean() << " : "
                << reception << std::endl;
...
const IReceptionResult *RadioMedium::computeReceptionResult(const IRadio *radio, const IListening *listening, const ITransmission *transmission) const
{
    ...
    if (!receptionDecision->isReceptionSuccessful())
    {
        ...
        outFile << "Latency: " << latency << " : "
                ...
                << "Transmitter Speed: " << speed << " m/s" << std::endl; // 속도 기록
        outFile.close();
...
const IReceptionResult *RadioMedium::getReceptionResult(const IRadio *radio, const IListening *listening, const ITransmission *transmission) const
{
    ...
    if (result)
        ...
        //std::cout << result->getReception() << std::endl;
...
IWirelessSignal *RadioMedium::createTransmitterSignal(const IRadio *radio, Packet *packet)
{
    ...
    //std::cout << "Packet : " << packet->getControlInfo() << std::endl;
