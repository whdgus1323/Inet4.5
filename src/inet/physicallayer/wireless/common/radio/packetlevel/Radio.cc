...
std::string pwdV = "/N";
std::string pwd =
        "C:/Users/Choe JongHyeon/Desktop/Temp"
        + pwdP + pwdD + pwdS + pwdV;*/



/*std::string pwdD = "/D200";
std::string pwdS = "/S30";
std::string pwdP = "/L";
std::string pwdN = "/23";
std::string pwd = "C:/Users/Choe JongHyeon/Desktop/Temp1" + pwdD + pwdS + pwdP + pwdN;*/



/*
std::string pwd =
        "D:/연구실/Applied Science/시뮬레이션 결과/Log/2025_07_14_AppSci_AODV_V10"
        + pwdP + pwdD + pwdS + pwdV;
        */
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        if (cbrLogEnabled) {
            ...
            if (simTime() > cbrRecordStartTime) {
                simtime_t elapsed = simTime() - cbrRecordStartTime;
                int64_t windowsPassed = static_cast<int64_t>(floor(elapsed.dbl() / cbrWindow.dbl()));
                cbrCurrentWindowStart = cbrRecordStartTime + windowsPassed * cbrWindow;
            }
            else
                cbrCurrentWindowStart = cbrRecordStartTime;
            cbrLastUpdateTime = simTime();
...
void Radio::endReception(cMessage *timer)
{
    ...
    if (timer == receptionTimer && isReceiverMode(radioMode) && arrival->getEndTime() == simTime()) {
        ...
        auto snir = medium->getSNIR(this, transmission);
        ...
        /*        if(logFlag) {
            if (!isReceptionSuccessful) {
                ...
                // 현재 노드의 위치 가져오기
                cModule *thisNode = getContainingNode(this);  // node[*]
                cModule *mobilityModule = thisNode->getSubmodule("mobility");
                ...
                // 로그 출력
                EV_WARN << "[RECEPTION FAILURE] SINR too low or interference.\n"
                        << "  - Time: " << simTime() << "\n"
                        << "  - SINR[min]: " << sinrMin << " dB\n"
                        << "  - Packet: " << packetName << "\n"
                        << "  - From Node: " << senderId << " → To Node: " << receiverId << "\n"
                        << "  - RX Position: (" << posX << ", " << posY << ", " << posZ << ")\n"
                        << endl;

                // CSV 저장
                std::ofstream out("data/sinr_drop_log.csv", std::ios::app);
                out << simTime() << ","    // TIME
                    << sinrMin << ","      // SINR_MIN
                    << packetName << ","   // PACKET
                    << senderId << ","     // TX_ID
        ...
        if (!isReceptionSuccessful) {
            ...
            if (mobilityModule) {
                inet::IMobility *mobility = check_and_cast<inet::IMobility *>(mobilityModule);
                thisPos = mobility->getCurrentPosition();
                posX = thisPos.x;
                posY = thisPos.y;
                posZ = thisPos.z;
            }
            ...
            for (int i = 0;; ++i) {
                ...
                cModule* node = getSimulation()->findModuleByPath(path.c_str());
                if (!node)
                    break;  // 더 이상 노드 없음
                totalCount++;
                ...
                inet::IMobility* mobility = check_and_cast<inet::IMobility*>(mob);
                inet::Coord pos = mobility->getCurrentPosition();
                if (thisPos.distance(pos) <= 50.0)
                    nearbyCount++;
            }
            ...
            EV_WARN << "[RECEPTION FAILURE] SINR too low or interference.\n"
                    ...
                    << "  - From Node: " << senderId << " → To Node: " << receiverId << "\n"
                    << "  - RX Position: (" << posX << ", " << posY << ", " << posZ << ")\n"
                    << "  - Nearby Nodes (<=50m): " << nearbyCount << " / " << totalCount << "\n"
                    << endl;

            // CSV 저장

            /*
            std::ofstream out(pwd + "/sinr_drop_log.csv", std::ios::app);
            static bool sinrDropWrite = true;
            if (sinrDropWrite) {
                out << "Time, SINR, Packet, sender, receiver, x, y, z, Nearby, Total\n";
                sinrDropWrite = false;
            }
            out << simTime() << ","    // TIME
                    << sinrMin << ","      // SINR_MIN
                    ...
                    << posY << ","         // RX_Y
                    << posZ << ","         // RX_Z
                    << nearbyCount << "," << totalCount  // NEARBY_COUNT, TOTAL_COUNT
                    << std::endl;
            out.close();
            */
        ...
        //std::cout << "******************* 패킷 드롭 발생: " << signal->getTransmission()->getPacket()->getName() << std::endl;
...
void Radio::advanceCbrTracking(simtime_t targetTime)
{
    ...
    simtime_t cursor = cbrLastUpdateTime;
    while (cursor < targetTime) {
        if (cursor < cbrRecordStartTime) {
            cursor = std::min(targetTime, cbrRecordStartTime);
            cbrLastUpdateTime = cursor;
            if (cursor >= cbrRecordStartTime)
                cbrCurrentWindowStart = cbrRecordStartTime;
            continue;
        }
        ...
        simtime_t segmentEnd = std::min(targetTime, windowEnd);
        if (segmentEnd > cursor && isChannelBusyForCbr())
            cbrBusyTime += segmentEnd - cursor;
...
{
    if (!cbrLogEnabled || cbrWindow <= SIMTIME_ZERO || simTime() < cbrRecordStartTime)
        return 0.0;

    advanceCbrTracking(simTime());

    simtime_t busyTime = cbrBusyTime;
    if (simTime() > cbrLastUpdateTime && isChannelBusyForCbr())
        busyTime += simTime() - cbrLastUpdateTime;
